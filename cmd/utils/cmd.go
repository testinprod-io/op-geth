...
// ExportTotalDifficulty exports difficulty into the specified file, truncating any data
// already present in the file.
func ExportTotalDifficulty(blockchain *core.BlockChain, fn string) error {
	log.Info("Exporting total difficulty", "file", fn)
	...
	// Iterate over the blocks and export them
	if err := blockchain.ExportTotalDifficulty(writer); err != nil {
		return err
	}
	log.Info("Exported difficulty", "file", fn)

	return nil
}

// ExportAppendTotalDifficulty exports difficulty into the specified file, appending to
// the file if data already exists in it.
func ExportAppendTotalDifficulty(blockchain *core.BlockChain, fn string, first uint64, last uint64) error {
	log.Info("Exporting total difficulty", "file", fn)
	...
	// Iterate over the blocks and export them
	if err := blockchain.ExportTotalDifficultyN(writer, first, last); err != nil {
		return err
	}
	log.Info("Exported total difficulty to", "file", fn)
